...
extract_cohort_counts_from_dynamic <- function(dyn, rel_col) {
  if("n_g" %in% names(dyn)){
    ...
    # Fallback: count non-missing estimates per rel_time
    cohort_counts <- dyn %>%
...
process_dynamic_file <- function(dynamic_path, out_dir) {
  dyn <- read_csv(dynamic_path, show_col_types = FALSE)
...
compute_from_dates <- function(out_dir) {
  ...
  treats <- load_treatment_dates(treat_path)
  
  analysis_data_path <- "data/02-analysis_data/processed/analysis_dataset.rds"
  date_range <- get_date_range(analysis_data_path)
