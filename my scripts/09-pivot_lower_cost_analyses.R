...
message("Starting lower-cost pivot analyses: glmer robustness + MDEs")
...
if (!"cohort" %in% names(df)) {
  ...
  message("Created cohort variable from treatment_group/treatment_date")
}
...
fit_glmer <- NULL
message("Attempting to fit glmer mixed-effects logistic model (may take a minute)...")
glmer_err <- NULL
tryCatch({
  fit_glmer <- glmer(
  ...
  message("Saved glmer fit to output/results/fit_cohort_glmer.rds")
  tidy_glmer <- broom.mixed::tidy(fit_glmer, effects = "fixed")
  write_csv(tidy_glmer, "output/results/fit_cohort_glmer_tidy.csv")
  message("Saved tidy glmer summary to output/results/fit_cohort_glmer_tidy.csv")
}, error = function(e) {
  glmer_err <<- conditionMessage(e)
  message("glmer failed: ", glmer_err)
})

if (is.null(fit_glmer)) {
  message("Falling back to a fixed-effects logistic approximation.")
  if (requireNamespace("fixest", quietly = TRUE)) {
    message("Using fixest::feglm as fallback (clustered SEs)")
    library(fixest)
    ...
    message("Saved fixest fallback fit to output/results/fit_cohort_fe.rds and tidy CSV")
  } else {
    message("fixest not available: falling back to GLM with province and cohort dummies (may be large).")
    ...
    message("Saved glm fallback fit to output/results/fit_cohort_glm_fe.rds and tidy CSV")
  }
}

# 2) MDE / power table (approximate two-sample proportion MDEs)
message("Computing approximate MDEs for binary outcome (individual-level approximation)")
...
message("Saved approximate MDE table to output/results/mde_table.csv")
...
message("Wrote diagnostics to output/results/pivot_diag.json")

message("Lower-cost pivot analyses complete. Results in output/results/")
